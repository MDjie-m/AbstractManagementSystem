...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        loginUserId: [
          { required: true, message: "登录账户id不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const storeUserIds = row.storeUserId || this.ids;
      this.$modal.confirm('是否确认删除门店员工编号为"' + storeUserIds + '"的数据项？').then(function() {
        return delUser(storeUserIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('billiard/user/export', {
        ...this.queryParams
