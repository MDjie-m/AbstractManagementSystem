<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="simNetStopListList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="断网原因" align="center" prop="cutReason" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="断网原因" prop="cutReason">
          <el-input v-model="form.cutReason" placeholder="请输入断网原因" />
        </el-form-item>
        <el-form-item label="数据日期" prop="yyyymmdd">
...
<script>
import { listSimNetStopList, getSimNetStopList, delSimNetStopList, addSimNetStopList, updateSimNetStopList } from "@/api/sim/simNetStopList";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSimNetStopList(this.queryParams).then(response => {
        this.simNetStopListList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSimNetStopList(provId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.provId != null) {
            updateSimNetStopList(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSimNetStopList(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除断网清单编号为"' + provIds + '"的数据项？').then(function() {
        return delSimNetStopList(provIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('sim/simNetStopList/export', {
        ...this.queryParams
      }, `simNetStopList_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
