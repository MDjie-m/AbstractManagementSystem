package com.renxin.consultant.controller.supervision;
...
public class ConsultantController extends BaseController
{
    ...
    @RateLimiter
    public AjaxResult getInfo(@RequestBody PsyConsult psyConsult)
    {
        return AjaxResult.success(psyConsultService.getOne(psyConsult.getId()));
    }

    @PostMapping(value = "/server/list")
    @RateLimiter
    public AjaxResult getServer(@RequestBody PsyConsultServeConfigReq req)
    {
        req.setStatus("0");
