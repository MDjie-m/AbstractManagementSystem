...
public class MyWsHandler extends AbstractWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        try {
            ...
            log.error("webSocket建立连接异常, session.getAttributes():" + session.getAttributes());
        }
    }
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message){
        try{
            ...
            log.error("webSocket消息处理异常, session.getAttributes():" + session.getAttributes() + "message.getPayload():" + message.getPayload());
        }
    }
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception)  {
        try{
            ...
            log.error("webSocket异常处理, 移除会话session.getId():" + session.getId());
        }
    }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        try{
            ...
            log.error("webSocket会话结束异常, 移除会话session.getId():" + session.getId());
        }
    }
